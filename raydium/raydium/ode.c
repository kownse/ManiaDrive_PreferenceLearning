...
__rayapi signed char raydium_ode_element_rotm_get (int e, dReal* mat)
{
    dReal *matrix;
    dReal c;
...
signed char raydium_ode_element_rotm_get_name (char *e, dReal* mat)
{
    return raydium_ode_element_rotm_get(raydium_ode_element_find(e),mat);
}

signed char raydium_ode_element_rot_forward_get (int e, dReal *dx, dReal *dy, dReal *dz)
{
    dReal *matrix;
    if(raydium_ode_element_isvalid(e))
    {
...
signed char raydium_ode_element_rot_forward_get_name (char *e, dReal *dx, dReal *dy, dReal *dz)
{
    return raydium_ode_element_rot_forward_get(raydium_ode_element_find(e), dx, dy, dz);
}
        ...
        for(i=0; i<RAYDIUM_ODE_MAX_JOINTS; i++)
            if(raydium_ode_joint[i].state)
            {
                ...
                case dJointTypeFixed:
                    ...
                    break;
                default:
                    raydium_log("ODE: ERROR: draw_all: invalid joint type !");
                }
        ...
        if(raydium_ode_element[i].state)
        {
            ...
            else
            {
                ...
                if(names==RAYDIUM_ODE_DRAW_DEBUG) // draw shape
                {
                    ...
                    else if(dGeomGetClass(raydium_ode_element[i].geom)==dSphereClass)
                    {
                        glutWireSphere(dGeomSphereGetRadius(raydium_ode_element[i].geom),10,10);
                    }
                    else if(dGeomGetClass(raydium_ode_element[i].geom)==dCapsuleClass)
                    {
