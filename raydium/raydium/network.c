...
int raydium_network_propag_add(int type, void *data, int size)
{
    ...
    for(i=0; i<RAYDIUM_NETWORK_MAX_PROPAGS; i++)
        if(!raydium_network_propag[i].state)
        {
            ...
            raydium_network_netcall_add((void*)raydium_network_propag_recv,type,1);
            return i;
        }
    ...
    if(type >= 0)
        for(i=0; i<RAYDIUM_NETWORK_MAX_NETCALLS; i++)
            if(raydium_network_netcall_type[i]==type)
            {
                ...
                f=(void (*)(int, char*))raydium_network_netcall_func[i];
                f(type,tmpbuff);
                done=1;
            }
    return done;
    ...
    if(raydium_network_mode==RAYDIUM_NETWORK_MODE_SERVER)
    {
        for(i=n=0; i<RAYDIUM_NETWORK_MAX_CLIENTS; i++)
            if(raydium_network_client[i] && now>raydium_network_keepalive[i]+RAYDIUM_NETWORK_TIMEOUT)
            {
                ...
                {
                    f=(void (*)(int))raydium_network_on_disconnect;
                    f(i);
                }
...
signed char raydium_network_init(void)
{
    ...
    if (ret)
    {
        raydium_log("network: FAILED ! (Winsock 2 Error: %i while asking for version 1.1)",ret);
        return(0);
    }
#endif
    ...
    raydium_network_netcall_add((void*)raydium_network_queue_ack_recv,RAYDIUM_NETWORK_PACKET_ACK,0);
...
signed char raydium_network_server_create_port(int port)
{
    if(port == 0)
        port = RAYDIUM_NETWORK_PORT;
    struct sockaddr_in sock;
    int on=1;
    int ret;
    ...
    raydium_log("network: server OK: waiting for clients (%i max) at udp port %i",RAYDIUM_NETWORK_MAX_CLIENTS,port);
    raydium_network_mode=RAYDIUM_NETWORK_MODE_SERVER;
    setsockopt(raydium_network_socket,SOL_SOCKET,SO_BROADCAST,(char *)&on,sizeof(on));
    raydium_network_set_socket_block(0);
    return(1);
...
// OnConnect:
    ...
    {
        f=(void (*)(int))raydium_network_on_connect;
        f(n);
    }
