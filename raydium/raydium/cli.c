...
raydium_init_argv=(char**)malloc(RAYDIUM_MAX_CLI_ARGS*sizeof(char *)); // can't use argc, since raydium.cli may add new argv's

for(i=0;i<argc;i++)
    {
    raydium_init_argv[i]=(char*)malloc(strlen(argv[i])+1);
...
if(fp)
    ...
    while(fgets(line,RAYDIUM_MAX_DIR_LEN,fp))
        ...
        while(raydium_parser_cut(line,arg,next,' '))
            {
            raydium_init_argv[raydium_init_argc]=(char*)malloc(strlen(arg)+1);
            strcpy(raydium_init_argv[raydium_init_argc],arg);
            raydium_init_argc++;
            strcpy(line,next);
            }
        raydium_init_argv[raydium_init_argc]=(char*)malloc(strlen(line)+1);
