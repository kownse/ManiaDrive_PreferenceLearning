...
if(raydium_register_variable_type[i]==RAYDIUM_REGISTER_INT)
    {
    p_i=(int*)raydium_register_variable_addr[i];
    (*p_i)=atoi(args);
    return i;
    }
...
if(raydium_register_variable_type[i]==RAYDIUM_REGISTER_SCHAR)
    {
    f_str=(char*)raydium_register_variable_addr[i];
    (*f_str)=atoi(args);
    return i;
    }
...
if(raydium_register_variable_type[i]==RAYDIUM_REGISTER_FLOAT)
    {
    float a;
    f_i=(float*)raydium_register_variable_addr[i];
    a=atof(args);
    (*f_i)=a;
    return i;
    }
...
raydium_register_function_list[raydium_register_function_index].handler=(void (*)(int, zval*, zval**, zval*, int, void***))addr;
