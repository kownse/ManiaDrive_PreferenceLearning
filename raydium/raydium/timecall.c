...
signed char raydium_timecall_devrtc_rate_change(unsigned long newrate)
{
#ifndef APPLE
#ifndef WIN32
        if(ioctl(raydium_timecall_devrtc_handle, RTC_IRQP_SET, newrate)==-1)
        {
                raydium_log("timecall: ERROR: changing /dev/rtc rate to %lu Hz failed !",newrate);
                raydium_log("timecall: is /proc/sys/dev/rtc/max-user-freq correct for this value ?");
                perror("system");
//                exit(errno);
                return 0;
        }
        raydium_log("timecall: /dev/rtc rate changed to %lu Hz",newrate);
        return 1;
#else
return 0;
#endif
...
raydium_timecall_add((void*)raydium_timecall_raydium,-1);
}
...
//    raydium_log("debug: need %i steps",steps);
    f=(void(*)())raydium_timecall_funct[i];
    ...
    else if(raydium_timecall_soft_call[i]) // this an "elastic-timed" callback
    {
     ...
     ff=(void (*)(GLfloat))raydium_timecall_funct[i];
