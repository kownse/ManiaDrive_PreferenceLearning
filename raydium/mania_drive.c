...
void build_gui_Lan(void)
{
    ...
    raydium_gui_widget_sizes(18,5,18);
    raydium_gui_button_create("btnMulti",handle,30,25,gettext("Connect"),btnNetworkConnect);
//    raydium_gui_widget_sizes(18,5,18);
...
//    raydium_gui_button_create("btnModeNetTracks",handle,25,55,gettext("Internet Tracks"),btnModeNetTracks);
    raydium_gui_button_create("btnModeLAN",handle,25,40,gettext("Let me in "),btnModeLan);
...
char collide(int e1, int e2, dContact *c)
{
    ...
    if(i1 == body_id && i2 == RAYDIUM_ODE_TAG_GROUND || i2 == body_id && i1 == RAYDIUM_ODE_TAG_GROUND)
    {
        attempinfo.collisiontime+=raydium_frame_time;

        if(bNoControl)
        {
            attempinfo.collisionnocontroltime += raydium_frame_time;
        }
    }
...
void display(void)
{
    ...
    if(vue==3 || vue==4 || vue==5 || vue==6)
    {
        ...
        if(raydium_joy_y>0.3)
        {
            ...
            bForward = 1;
            bBack = 0;
            attempinfo.forwardtime += raydium_frame_time;
        }
        else if(raydium_joy_y<0.3)
        {
            ...
            if(raydium_joy_y < 0)
            {
                bBack = 1;
                bForward = 0;
                attempinfo.backtime += raydium_frame_time;
            }
            else
            {
                bBack = 0;
        ...
        if(raydium_joy_x > 0)
        {
            bTurnLeft = 1;
            bTurnRight = 0;
            attempinfo.turnrighttime += raydium_frame_time;
            if(bForward == 1)
            {
                attempinfo.forwardrighttime += raydium_frame_time;
            }
            else if(bBack == 1)
            {
                attempinfo.backrighttime += raydium_frame_time;
            }
        }
        else if(raydium_joy_x < 0)
        {
            bTurnRight = 1;
            bTurnLeft = 0;
            attempinfo.turnlefttime += raydium_frame_time;

            if(bForward == 1)
            {
                attempinfo.forwardlefttime += raydium_frame_time;
            }
            else if(bBack == 1)
            {
                attempinfo.backlefttime += raydium_frame_time;
            }
        }
        else
        {
            ...
            {
                bNoControl = 1;
                attempinfo.nocontroltime += raydium_frame_time;
            }
    ...
    */
    ...
    attempinfo.attempttime += raydium_frame_time;
    if(!bLeap)
    {
        attempinfo.leaptime += raydium_frame_time;
        if(bNoControl == 1)
        {
            attempinfo.leapnocontroltime += raydium_frame_time;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    raydium_network_netcall_add(prelearn_netcall_receive_levelid, S2C_SEND_LEVELID, 1);
    raydium_network_netcall_add(prelearn_netcall_receive_sessionid, S2C_SEND_SESSIONID, 1);
    raydium_network_netcall_add(prelearn_get_connect_server_port, S2C_SEND_SERVER_PORT, 1);
    raydium_network_netcall_add(session_end, S2C_SESSION_END, 1);
    raydium_network_netcall_add(net_new_session, S2C_READY_NEW_SESSION, 1);
