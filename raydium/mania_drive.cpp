...
#include "PreferenceModeling/DataCollect/CollectGUI.h"
#include "PreferenceModeling/DataCollect/CollectNet.h"
...
void netcall_remaining_time(int type, char *buff)
{
    ...
    partytime=f;
    if(levelinfo.partytime == 0 && partytime > 10.0f)
        levelinfo.partytime = partytime;
}

void session_end(int type, char *buff)
{
    raydium_log("session_end");
    change_game_state(GAME_MENU);
    mni_current[0]=0;
    message[0]=0;
...
//        track_score.time=get_score(mni,track_score.player);
    ...
    prelearn_gameinfo_level_reset(mni);
...
void btnKeyHelpOk(raydium_gui_Object *w)
{
    ...
    raydium_parser_db_set("ManiaDrive-JoystickEnabled",const_cast<char*>((joystick_enabled?"y":"n")));
...
void AfterFade(void)
{
    ...
    if(!mni_load(mni_next))
    {
        ...
        raydium_gui_button_create("btnErrorOk",handle,35,15,"OK",(void*)btnBackToMainMenu);
    ...
    if(mode == MODE_SOLO)
    {
        partytime = trackdata.gold_time * 5;    //以3倍过关时间为准
        ++level_id;
        levelinfo.level_id = level_id;
        levelinfo.partytime = partytime;
        raydium_log("开始单机收集 level_id=%d", level_id);
    }
    mni_next[0]=0;
}
...
void btnCantDrive(raydium_gui_Object *w)
{
    ...
    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnCantDriveOk",handle,35,15,"OK",(void*)btnCantDriveOk);
}
...
void btnKeyHelp(raydium_gui_Object *w)
{
    ...
    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnCantDriveOk",handle,35,5,gettext("OK"),(void*)btnKeyHelpOk);
}
...
void btnNetworkConnect(raydium_gui_Object *w)
{
    ...
    raydium_gui_read_name("menu","edtPlayerName",raydium_network_name_local);
    //GetPCName(raydium_network_name_local);
...
void btnSearchLAN(raydium_gui_Object *w)
{
    ...
    for(i=0; i<n; i++)
    {
        ...
        raydium_gui_zone_create("*zone",handle,0,y-2,100,5,i,(void*)btnServerClick);

        y-=6;
    }

    raydium_gui_widget_sizes(3,3,12);
    raydium_gui_button_create("btnBackToMain",handle,2,5,"<",(void*)btnBackToMainMenu);
}
...
void gui_menu_BestTime(raydium_gui_Object *w)
{
    ...
    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnBestTimeOkOk",handle,35,15,"OK",(void*)btnBestTimeOk);
}
...
void build_gui_NeedRestart(void)
{
    ...
    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnRestartOk",handle,35,15,"OK",(void*)btnRestartClick);
}
...
void build_gui_StoryCompleted(void)
{
    ...
    raydium_gui_button_create("btnStoryCompletedOk",handle,15,15,gettext("OK"),(void*)btnStoryCompletedOk);
    raydium_gui_button_create("btnCredits",handle,55,15,gettext("Credits !"),(void*)btnCredits);
...
void build_gui_InternetTracks(void)
{
    ...
    raydium_gui_widget_sizes(25,4,18);
    raydium_gui_button_create("btnBestTime",handle,47,55,gettext("Get best time"),(void*)gui_menu_BestTime);

    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnDrive",handle,35,5,gettext("Drive !"),(void*)btnDriveNet);
...
void build_gui_Solo(void)
{
    ...
    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnDrive",handle,35,5,gettext("Drive !"),(void*)btnOtherTracksSolo);
...
signed char build_gui_Story_sub(int handle, char *filename, float x1, float x2, int id)
{
    ...
    for(i=0; i<len; i++)
        if(ret[i]=='\n' || ret[i]==0)
        {
            ...
            candrive=(part2[1]!='4');
            raydium_gui_button_create(part1,handle,x1,pos,(candrive?gettext("Drive"):gettext("-")),(void*)(candrive?btnDriveSolo:btnCantDrive));
...
void build_gui_Story(void)
{
    ...
    if(completed)
    {
        ...
        raydium_gui_AfterGuiDrawCallback=(void*)after_gui;
        raydium_gui_window_OnDelete_name("menu",(void*)video_beg_delete);
    ...
    raydium_gui_widget_sizes(6,3,14);
    raydium_gui_button_create("btnBackToMain",handle,5,3,gettext("<"),(void*)btnBackToMainMenu);
...
void build_gui_Lan(void)
{
    ...
    raydium_gui_widget_sizes(18,5,18);
    raydium_gui_button_create("btnMulti",handle,30,25,gettext("Connect"),(void*)btnNetworkConnect);
//    raydium_gui_widget_sizes(18,5,18);
...
void build_gui_Options(void)
{
    ...
    raydium_gui_button_create("btnEngineVolTest",handle,81,51,gettext("test"),(void*)btnEngineVolTest);
    ...
    raydium_gui_widget_sizes(9,3,14);
    raydium_gui_button_create("btnMusicVolTest",handle,81,41,gettext("test"),(void*)btnMusicVolTest);
...
// save this settings
    ...
    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnApply",handle,30,5,gettext("Apply"),(void*)btnApplyOptions);


    raydium_gui_widget_sizes(15,5,18);
    raydium_gui_button_create("btnKeyHelp",handle,65,5,gettext("Controls"),(void*)btnKeyHelp);
...
void showMessage(char *file, int id)
{
    ...
    if(found)
    {
        ...
        raydium_gui_widget_sizes(15,5,18);
        id=raydium_gui_button_create(var,handle,40,100-((float)100/(nlines+2))*(i+1.5),"OK",(void*)btnMessageOk);
...
void build_gui_Main(void)
{
    ...
    raydium_gui_widget_sizes(0,0,18);
    raydium_gui_label_create("lblMode",handle,50,90,gettext("- Gameplay Research - "),0,0,0);

    raydium_gui_widget_sizes(27,4,18);
    //raydium_gui_button_create("btnModeStory",handle,25,70,gettext("Story"),(void*)btnModeStory);
    //raydium_gui_button_create("btnModeStory",handle,25,55,gettext("Local Model"),(void*)prelearn_btnLocalCollect);
    raydium_gui_button_create("btnModeStory",handle,25,55,gettext("\341 \342 \345 \346"),(void*)prelearn_btnLocalCollect);
//    raydium_gui_button_create("btnModeNetTracks",handle,25,55,gettext("Internet Tracks"),btnModeNetTracks);
    raydium_gui_button_create("btnModeLAN",handle,25,40,gettext("\343 \344 \345 \346"),(void*)btnModeLan);
//    raydium_gui_button_create("btnTest",handle,25,25,gettext("Test 4AFC"),btnTest4AFC);
    raydium_gui_button_create("btnOptions",handle,25,25,gettext("\332 \333"),(void*)btnOptions);

    raydium_gui_widget_sizes(8,3,14);
    raydium_gui_button_create("btnBackToMain",handle,4,5,gettext("Quit"),(void*)btnQuit);
...
void frame_step(GLfloat step)
{
    ...
    if(old>0 && partytime==0) // end of this party
    {
        ...
        else if(mode == MODE_SOLO)
        {
            ...
            {
                //第一次partytime为0给出倒计时并跳转到GAME_SCORE以给出GAME OVER的画面
                partytime = 3;
                change_game_state(GAME_SCORE);
            }
            else
            {
                ...
                /*
                //第二次partytime为0就应该考虑换地图或者重新获取偏好结果了
                if(level_id >=1 )
                {
                    level_id = -1;
                    //btnNewGameSession(0);
...
//        {
        if(timer<=trackdata.gold_time)
        {
            ...
            if(mode==MODE_SOLO)
            {
                partytime = 0.1f;       //仅仅是为了缓冲
            }

            attempinfo.bFinished = 2;
            bPassed = true;
        }
        else
        {
...
//        }
        ...
        //统计attempt信息
        prelearn_gameinfo_attempt_finished();
    }
    ...
    if(type==GAME_SCORE)
    {
        if(old != GAME_END)
        {
            ...
            prelearn_gameinfo_attempt_over(total, n);
        }
...
void draw_timer(void)
{
    ...
    if(mode==MODE_MULTI)
    {
        ...
        raydium_osd_color_change(0.89,0.85,0.66);
        raydium_osd_printf(20,1,12,0.5,"font-impact.tga",gettext(" party time:"));
        raydium_osd_printf(33,1,12,0.7,"font-lcdmono.tga","^f%i:%02i:%02i:%03i",h2,m2,s2,ms2);
...
void draw_best_score(void)
{
    ...
    if(game_state==GAME_SCORE)
    {
        if(!bPassed)
        {
            raydium_osd_printf(35+SHADOW_OFFSET,40-SHADOW_OFFSET,40,0.5,"font-impact.tga",gettext("^0Game over !"));
            raydium_osd_printf(35,40,40,0.5,"font-impact.tga",gettext("^cGame over !"));
        }
        else
        {
            raydium_osd_printf(35+SHADOW_OFFSET,40-SHADOW_OFFSET,40,0.5,"font-impact.tga",gettext("^0You Win !"));
            raydium_osd_printf(35,40,40,0.5,"font-impact.tga",gettext("^cYou Win !"));
        }
    ...
    if(track_score.time>=0)
    {
        ...
        raydium_osd_color_change(0.89,0.85,0.66);
        raydium_osd_printf(20,4,12,0.5,"font-impact.tga",gettext("     record:"));
        raydium_osd_printf(33,4,12,0.7,"font-lcdmono.tga","^e%i:%02i:%02i:%03i",h,m,s,ms);
        raydium_osd_printf(39,4,12,0.5,"font-impact.tga","^f- %s",track_score.player);
        */

        hms(yourbest,&h,&m,&s,&ms);
        raydium_osd_color_change(0.89,0.85,0.66);
        raydium_osd_printf(20,4,12,0.5,"font-impact.tga",gettext("  your best:"));
        raydium_osd_printf(33,4,12,0.7,"font-lcdmono.tga","^f%i:%02i:%02i:%03i",h,m,s,ms);
    }
...
char rays_collide(int e1, int e2, dContact *c)
{
    int i1, i2;
...
char collide(int e1, int e2, dContact *c)
{
    ...
    //if(IS_CAR_BODY(i1) && i2 == RAYDIUM_ODE_TAG_GROUND || IS_CAR_BODY(i2) && i1 == RAYDIUM_ODE_TAG_GROUND)
    if(game_state == GAME_GAME)
    {
        //raydium_log("collide func");
        //判断悬空
        if( (i1 == TYPE_CAR && i2 == RAYDIUM_ODE_TAG_GROUND) || (i2 == TYPE_CAR && i2 == RAYDIUM_ODE_TAG_GROUND) )
        {
            bLeap = 0;
            ++nColWhell;
        }
...
char draw_element_after(int elem)
{
    ...
    if(IS_CAR_BODY(type) && raydium_ode_network_element_isdistant(elem))
    {
        ...
        raydium_ode_element[elem].mesh=raydium_object_find_load(const_cast<char*>((type==TYPE_CAR_BODY)?"clio_wheels.tri":"clio_sp_wheels.tri"));
    }
    return 1;
}
...
//    raydium_sound_SourceStop(sound_car);
    ...
    //build_gui_Story();
        ...
        build_gui_InternetTracks();
    else if(old==MODE_MULTI)
        build_gui_Lan();
    else if(old==MODE_OTHERS)
        build_gui_Solo();
...
void display(void)
{
    ...
    if(scroll>=0)
    {
        ...
        if(cpt==0 && scroll!=99999)
        {
            ...
            GLfloat to[4]= {0,0,0,1};
            scroll=99999; // ugly :)
            raydium_osd_fade_from(from,to,1,(void*)credits_stop);
        }
        raydium_rendering_finish();
    ...
    if(raydium_key_last==1032)
    {
        if(!bPassed)
        {
            if(game_state == GAME_GAME)
            {
                ...
                //attempinfo.finishProportion = (float)n / total;
...
int run(int argc, char **argv)
{
    ...
    */
    ...
    raydium_sound_SetSourceGain(sound_car,1);  // Engine Gain
    ...
    raydium_timecall_add((void*)frame_step,-1);
    change_game_state(GAME_MENU);
    message[0]=0;
    mode=MODE_NONE;

    raydium_sound_music_eof_callback=(int (*)(char*))music_playlist;
    raydium_sound_music_changed_callback=(void (*)())music_change;
    raydium_ode_AfterElementDrawCallback=(void*)draw_element_after;
    raydium_ode_CollideCallback=(void*)collide;
    raydium_ode_ObjectNearCollide=(void*)nearcall;
    raydium_ode_RayCallback = (void*)rays_collide;
    ...
    raydium_network_netcall_add((void*)restart_asked_from_network,NET_RESTART,1);
    raydium_network_netcall_add((void*)netcall_mni_change,NET_CHANGE_MAP,1);
    raydium_network_netcall_add((void*)netcall_remaining_time,NET_REMAINING,1);
    raydium_network_netcall_add((void*)prelearn_netcall_receive_levelid, S2C_SEND_LEVELID, 1);
    raydium_network_netcall_add((void*)prelearn_netcall_receive_sessionid, S2C_SEND_SESSIONID, 1);
#ifndef DIRECT_DEBUG
    raydium_network_netcall_add((void*)prelearn_get_connect_server_port, S2C_SEND_SERVER_PORT, 1);
#endif
    raydium_network_netcall_add((void*)session_end, S2C_SESSION_END, 1);
    raydium_network_netcall_add((void*)net_new_session, S2C_READY_NEW_SESSION, 1);
